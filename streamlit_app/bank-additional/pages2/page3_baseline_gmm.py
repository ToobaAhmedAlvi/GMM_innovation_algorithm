...
st.title("📈 Step 3: Baseline Gaussian Mixture Model")

# Top KPIs (initially empty, will update after training)
...
with st.expander("ℹ️ Parameter Guide", expanded=False):
    st.markdown("""
    **Number of Clusters (K):** How many customer segments to create  
    **Covariance Type:** Shape of cluster distributions  
    - `full`: Ellipsoids with any orientation (most flexible)  
...
with col1:
    n_clusters = st.number_input(
        ...
        value=recommended_k,
        help="Recommended value from Step 2"
    )
...
with col2:
    covariance_type = st.selectbox(
        ...
        index=0,
        help="Shape of cluster distributions"
    )
...
if 'baseline_trained' in st.session_state:
    ...
    fig.add_hline(y=df['y_binary'].mean()*100, 
    ...
    with col2:
        metrics_df = pd.DataFrame({
            'Metric': ['Silhouette Score', 'Davies-Bouldin Index', 
                      'Calinski-Harabasz Score', 'Training Time (s)'],
...
with col2:
    if 'baseline_trained' in st.session_state:
        if st.button("➡️ Proceed to Innovative Deep GMM", 
                    use_container_width=True, type="primary"):
            st.switch_page("pages/page4_innovative_gmm.py")
    else:
        st.warning("⚠️ Please train the baseline model first")
